...
// let API_URL = "https://api.us.apiconnect.ibmcloud.com/spbodieusibmcom-kenishia/sb/";

function getPatientInfo(url, patientID) {
  return new Promise(function (resolve, reject) {
    request(
      url + "getInfo/patients/" + patientID,
      function (error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          let patientInfo = {
            name:
              body["HCCMAREA"]["CA_PATIENT_REQUEST"]["CA_FIRST_NAME"] +
              " " +
              body["HCCMAREA"]["CA_PATIENT_REQUEST"]["CA_LAST_NAME"],
...
function getPatientMedications(url, patientID) {
  return new Promise(function (resolve, reject) {
    request(
      url + "getInfo/prescription/" + patientID,
      function (error, response, body) {
        let medications = [];
        if (!error && response.statusCode == 200) {
          body = JSON.parse(body);
          ...
          try {
            let medicationData =
              body["GETMEDO"]["CA_LIST_MEDICATION_REQUEST"]["CA_MEDICATIONS"];
          } catch (e) {
            return resolve(medications);
          }

          for (let i = 0, len = medicationData.length; i < len; i++) {
...
function getPatientAppointments(url, patientID) {
  return new Promise(function (resolve, reject) {
    request(
      url + "appointments/list/" + patientID,
      function (error, response, body) {
        let appointments = [];
        if (!error && response.statusCode == 200) {
          body = JSON.parse(body);
          ...
          for (let i = 0, len = appointmentsData.length; i < len; i++) {
            appointments.push(
              appointmentsData[i]["APPT_DATE"].trim() +
                " " +
                appointmentsData[i]["APPT_TIME"]
                  .trim()
                  .replace(".", ":")
                  .substring(0, 5) +
                " - " +
                appointmentsData[i]["MED_FIELD"].trim()
            );
          }
  ...
  return new Promise(function (resolve, reject) {
    request(url + "listObs/" + patientID, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        let measurementsData = body["ResultSet Output"];

        for (let i = 0, len = measurementsData.length; i < len; i++) {
          switch (measurementsData[i][["DESCRIPTION"]]) {
            case "Tobacco smoking status NHIS":
              if (!measurements.smokerstatus) {
                measurements.smokerstatus =
                  measurementsData[i]["CHARACTERVALUE"];
              }
              break;
            case "Body Height":
...
function patientLogin(url, username, password) {
  return new Promise(function (resolve, reject) {
    request.post(
      { url: url + "login/user", json: { UID: username, PASS: password } },
      function (error, response, body) {
        if (!error && response.statusCode == 200) {
